...
import { LegalCategory, LegalRequest, LegalResponse } from '../types/legal';
...
class GeminiService {
  ...
  constructor() {
    try {
      ...
      throw new Error('שגיאה באתחול המערכת. אנא בדוק את הגדרות ה-API.');
    }
  }

  private getCategoryPrompt(category: LegalCategory, details: Record<string, string>): string {
  ...
  async getLegalResponse(request: LegalRequest): Promise<LegalResponse> {
    if (!categoryService.validateCategoryInput(request.category, request.details)) {
      throw new Error('חסרים פרטים נדרשים');
    }

    try {
      const categoryPrompt = this.getCategoryPrompt(request.category, request.details);
      
      const prompt = `
        ${this.legalContext}
        
        קטגוריה: ${categoryService.getCategoryById(request.category)?.title}
        ${categoryPrompt}
        ${request.sourceUrl ? '\nמקור מידע נוסף: ' + request.sourceUrl : ''}
      ...
      throw new Error('מצטערים, אירעה שגיאה בקבלת המידע המשפטי. אנא נסו שוב מאוחר יותר.');
    }
  }
}
